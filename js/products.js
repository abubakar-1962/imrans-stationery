...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  function buildQuickView() {
      ...
      '<div class="qv-body">' +
        ...
        '<div class="qv-row">' +
          ...
          '<div class="qv-reviews">' +
            ...
            '<form class="qv-review-form">' +
              '<h4>Write a Review</h4>' +
              '<input type="text" name="rname" placeholder="Your name" required>' +
              '<select name="rrating" required><option value="5">5 Stars</option><option value="4">4 Stars</option><option value="3">3 Stars</option><option value="2">2 Stars</option><option value="1">1 Star</option></select>' +
              '<textarea name="rcomment" placeholder="Your review" required></textarea>' +
    ...
    revForm.addEventListener("submit", function(e) {
      e.preventDefault();
      var name = revForm.elements["rname"].value;
      var rating = revForm.elements["rrating"].value;
      var comment = revForm.elements["rcomment"].value;
      var date = new Date().toLocaleDateString();
      var newReview = { name: name, rating: parseInt(rating), comment: comment, date: date };
      
      // Append to local storage since there is no backend yet.
      // NOTE: Reviews won't be visible to other visitors until there's a real backend.
      var localReviews = JSON.parse(localStorage.getItem("ims_reviews_" + qv.product.id) || "[]");
      localReviews.push(newReview);
      localStorage.setItem("ims_reviews_" + qv.product.id, JSON.stringify(localReviews));
      
      renderReviews(qv.product);
      revForm.reset();
    });
  }
    ...
    var localReviews = JSON.parse(localStorage.getItem("ims_reviews_" + p.id) || "[]");
    var serverReviews = p.reviews || [];
    var combined = serverReviews.concat(localReviews);
    ...
    combined.forEach(function(r) {
      ...
      d.innerHTML = "<strong>" + r.name + "</strong> (" + r.rating + "/5 stars) <small>" + r.date + "</small><p>" + r.comment + "</p>";
      frag.appendChild(d);
    });
